...
import { requireExternalKey, getLyrics } from "../../../_lib/collab-store.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { record, storage } = await getLyrics(externalTrackId);
    if (!record) return res.status(404).json({ error: "Lyric draft not found" });
    return res.status(200).json({ ok: true, storage, item: record });
  } catch (e) {
    console.error("collab lyrics get error:", e);
